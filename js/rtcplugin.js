...
// TODO: refactor to use EventEmitter - as this is not concurrency safe.
// i.e. Offer is made after first offer, but before the first can come back - clobbers onHandleOffer with last value.
(function (window) {
    if ("ActiveXObject" in window) {
        ...
        /* private */ function nativeEventHandler(data) {
            ...
            if (json.type == 'offer') {
                if (this.onCreateOffer){
                    this.onCreateOffer(json);
                }
            } else if (json.type == 'answer') { //back from C++
                if(this.onHandleOffer) {
                    this.onHandleOffer(json);
        ...
        */
        RTCPlugin.prototype.setupRenderSurface = function (easyRtcId) {
            console.log("Adding render surface for " + easyRtcId);
